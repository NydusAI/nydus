...
- Spawner extraction (OpenClaw + ZeroClaw)
- Hatcher output (OpenClaw + ZeroClaw)
...
@pytest.fixture
def zeroclaw_project_with_mcp(tmp_path: Path) -> Path:
    """ZeroClaw project directory containing MCP configs."""
    root = tmp_path / "zeroclaw_mcp"
    root.mkdir()
    (root / "persona.md").write_text("I solve problems.\n")
    (root / ".zeroclaw").mkdir()
    mcp_dir = root / "mcp"
    mcp_dir.mkdir()
    (mcp_dir / "filesystem.json").write_text(
        json.dumps({"command": "npx", "args": ["-y", "@anthropic/filesystem-mcp"]})
    )
    return root
...
class TestMcpServerConfigSchema:
    ...
    def test_skills_module_with_configs(self):
        module = SkillsModule(
            mcp_configs={"db": McpServerConfig(command="node")}
        )
...
class TestMcpPackagerRoundTrip:
    def test_mcp_configs_written_to_archive(self, mcp_egg: Egg, tmp_path: Path):
        path = pack_with_raw(mcp_egg, tmp_path / "test.egg", {})
        ...
        egg = Egg(
            ...
            skills=SkillsModule(
                skills=[
                    SkillRecord(
                        id="s1", name="test",
                        source_type="x", content="hello",
                    )
                ]
            ),
        )
        path = pack_with_raw(egg, tmp_path / "test.egg", {})
        loaded = unpack(path)
...
class TestOpenClawMcpExtraction:
    def test_parse_from_mcp_dir(self):
        ...
        files = {
            ...
            "skill.md": "# greet\n\nSay hello.\n",
            "mcp/snowflake.json": json.dumps({"command": "npx", "args": ["-y", "@anthropic/snowflake-mcp"]}),
        }
...
class TestZeroClawMcpExtraction:
    def test_parse_from_mcp_dir(self):
        from pynydus.agents.zeroclaw.spawner import ZeroClawSpawner

        files = {
            "persona.md": "I solve problems.\n",
            "mcp/filesystem.json": json.dumps({"command": "npx", "args": ["-y", "@anthropic/filesystem-mcp"]}),
        }
        result = ZeroClawSpawner().parse(files)
        assert "filesystem" in result.mcp_configs
        assert result.mcp_configs["filesystem"]["command"] == "npx"
...
class TestOpenClawHatcherMcp:
    def test_mcp_configs_written(self, mcp_egg: Egg, tmp_path: Path):
        from pynydus.agents.openclaw.hatcher import OpenClawHatcher

        result = OpenClawHatcher().hatch(mcp_egg, tmp_path / "out")
    ...
    def test_no_mcp_configs_skips_dir(self, tmp_path: Path):
        from pynydus.agents.openclaw.hatcher import OpenClawHatcher
        ...
        egg = Egg(
            ...
            memory=MemoryModule(
                memory=[
                    MemoryRecord(
                        id="m1", text="pref", label=MemoryLabel.PERSONA,
                        source_framework="x", source_store="x",
                    )
                ]
            ),
        )
        result = OpenClawHatcher().hatch(egg, tmp_path / "out")
        assert not (tmp_path / "out" / "mcp").exists()
        assert not any(f.startswith("mcp/") for f in result.files_created)


class TestZeroClawHatcherMcp:
    def test_mcp_configs_written(self, mcp_egg: Egg, tmp_path: Path):
        from pynydus.agents.zeroclaw.hatcher import ZeroClawHatcher

        egg = mcp_egg.model_copy()
        egg.manifest.source_type = SourceType.ZEROCLAW
        result = ZeroClawHatcher().hatch(egg, tmp_path / "out")
        mcp_dir = tmp_path / "out" / "mcp"
        assert mcp_dir.is_dir()
        assert (mcp_dir / "snowflake.json").exists()
        assert "mcp/snowflake.json" in result.files_created
    ...
    def test_no_mcp_configs_skips_dir(self, tmp_path: Path):
        ...
        egg = Egg(
            ...
            memory=MemoryModule(
                memory=[
                    MemoryRecord(
                        id="m1", text="persona text", label=MemoryLabel.PERSONA,
                        source_framework="x", source_store="x",
                    )
                ]
            ),
...
# ---------------------------------------------------------------------------
# End-to-end: spawn → pack → unpack → hatch
# ---------------------------------------------------------------------------


class TestMcpEndToEnd:
    def test_openclaw_spawn_pack_unpack_hatch(self, tmp_path: Path):
        from pynydus.agents.openclaw.hatcher import OpenClawHatcher
        from pynydus.agents.openclaw.spawner import OpenClawSpawner
        ...
        egg = Egg(
            ...
            skills=SkillsModule(
                skills=[
                    SkillRecord(
                        id="s1", name="greet",
                        source_type="openclaw", content="Say hello.",
                    )
                ],
                mcp_configs={
                    name: McpServerConfig(**cfg)
                    for name, cfg in result.mcp_configs.items()
                },
            ),
            memory=MemoryModule(
                memory=[
                    MemoryRecord(
                        id="m1", text="I am helpful.", label=MemoryLabel.PERSONA,
        ...
        egg_path = pack_with_raw(egg, tmp_path / "test.egg", files)
        loaded = unpack(egg_path)
        assert len(loaded.skills.mcp_configs) == 1
        assert loaded.skills.mcp_configs["snowflake"].command == "npx"

        out = tmp_path / "hatched"
        hatch_result = OpenClawHatcher().hatch(loaded, out)
