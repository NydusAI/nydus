    ...
    def test_egg_with_required_secrets_is_valid(self):
        ...
        egg = _make_egg(
            secrets=SecretsModule(
                secrets=[_make_secret("api_key", required_at_hatch=True)]
            ),
        )
        report = validate_egg(egg)
        assert report.valid is True
...
class TestSecretValidation:
    def test_value_present_true_is_error(self):
        ...
        egg = _make_egg(
            secrets=SecretsModule(
                secrets=[_make_secret("leak", value_present=True)]
            ),
        )
        report = validate_egg(egg)
        assert report.valid is False
    ...
    def test_value_present_location_field(self):
        egg = _make_egg(
            secrets=SecretsModule(
                secrets=[_make_secret("oops", value_present=True)]
            ),
        )
        report = validate_egg(egg)
        errors = [i for i in report.issues if i.level == "error"]
...
class TestSkillRefValidation:
    def test_valid_skill_ref(self):
        egg = _make_egg(
            ...
            memory=MemoryModule(
                memory=[_make_memory_with_skill_ref("m1", "test_skill")]
            ),
        )
        report = validate_egg(egg)
    ...
    def test_invalid_skill_ref_warns(self):
        egg = _make_egg(
            skills=SkillsModule(skills=[_make_skill("s1")]),
            memory=MemoryModule(
                memory=[_make_memory_with_skill_ref("m1", "nonexistent_skill")]
            ),
        )
        report = validate_egg(egg)
        assert report.valid is True
