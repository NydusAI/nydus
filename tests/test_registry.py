...
from pynydus.pkg.config import NydusConfig, RegistryConfig, load_config
...
class TestNydusConfigRegistry:
    ...
    def test_load_config_with_registry(self, tmp_path: Path):
        data = {
            "registry": {
                "url": "http://nest.example.com",
                "author": "jae",
            }
        }
    ...
    def test_load_config_registry_only(self, tmp_path: Path):
        """Config with only registry section (no LLM)."""
        data = {"registry": {"url": "http://localhost:8000"}}
        p = tmp_path / "config.json"
        p.write_text(json.dumps(data))
        cfg = load_config(p)
        assert cfg.registry is not None
...
class TestSDKPushPull:
    def test_push_without_registry_config_raises(self, tmp_path: Path):
    ...
    def test_sdk_push_delegates_to_nest_client(
        ...
        cfg_path.write_text(json.dumps({
            "registry": {"url": "http://localhost:8000", "author": "jae"}
        }))
...
class TestNestClientAuth:
    ...
    def test_login_success_stores_token(
        self, mock_post: MagicMock, mock_save: MagicMock
    ):
        mock_resp = MagicMock()
        mock_resp.status_code = 200
