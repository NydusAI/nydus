...
class TestAgentSkill:
    ...
    def test_full(self):
        s = AgentSkill(
            ...
            version="2.0",
            license="MIT",
            compatibility=["openclaw", "letta"],
            metadata={"tags": ["social", "intro"], "source_framework": "openclaw"},
            body="# Greeting\nSay hello nicely.",
        )
        assert s.name == "greet"
        assert len(s.compatibility) == 2
        assert s.metadata["tags"] == ["social", "intro"]
...
class TestAgentSkillToSkillRecord:
    def test_basic_conversion(self):
        ...
        data = agent_skill_to_skill_record(skill, skill_id="sk-1", source_type="openclaw")
    ...
    def test_roundtrip_record_to_agent_to_record(self):
        ...
        data = agent_skill_to_skill_record(agent, skill_id="sk-rt", source_type="letta")
